...
public class LogGeneratorController implements Initializable {
    ...
    @FXML
    private TextArea logsResultTextArea;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        HBox.setMargin(logMessageLabel, new Insets(15, 5, 5, 0));

        logsResultTextArea.setPrefRowCount(128);
    ...
    private void handleGenerate(@SuppressWarnings("unused") ActionEvent actionEvent) {
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            Platform.runLater(() -> {
                setRunning(false);
                logMessageLabel.setText(e.getMessage());
            });
            return null;
    ...
    private void write(String line) {
        if (printConsoleCheckBox.isSelected()) {
            Platform.runLater(() -> {
                logsResultTextArea.appendText(line);
                logsResultTextArea.appendText("\n");
            });
        }
        if (writer != null) {
            writer.println(line);
